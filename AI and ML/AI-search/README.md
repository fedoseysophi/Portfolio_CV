...
Corporate Documents QA API - a set of Python services for building and running a Question Answering (QA) API over a corporate document collection. The system enables users to search for relevant documents and receive concise, grounded answers generated from your organization's PDF and text files.
...
1. The `/search` endpoint returns the top-N relevant documents
2. The `/initial-answer` endpoint generates a concise answer using only the selected document's content
3. The system enforces tight answer grounding: if information cannot be found, the model clearly replies "Cannot answer"
4. `/followup` supports clarification questions based on the context already provided
...
1. Prepare your document corpus and embeddings using pipeline scripts
2. Configure models and path locations as needed
